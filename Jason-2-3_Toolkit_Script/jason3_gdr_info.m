...
function jason3_gdr_info( In_Pass, lat_range, File_Suffix_name )
%JASON2_GDR_EXTRACT Summary of this function goes here
%   Detailed explanation goes here
%------------------------------------------%
% To read Jason GDR data version D
% % Input areguments are ( PassNumber as String ,LatitudeRange as 1x2 array,File Suffix name in
%  string)
...
% 2013.13.29
% Modified by Dolu Okeowo: 03/23/2016
% Modified by Chihung Chang: 03/20/2017
%------------------------------------------%
...
filename1 = strcat(outfolder,'/j3_gdr_p', In_Pass, '_',File_Suffix_name,'_info.txt');
...
count=0;
NoCycle=length(Cycle_Folders)-2;
for i=3:length(Cycle_Folders)
    count=count+1;
    Fraction= floor((count/NoCycle)*100);
